...
for f in index.html gallery.html analysis.html widget.html widget-ds.html embed.html host-page.html host-home.html; do
  [ -f "$f" ] || continue
  /usr/bin/sed -i '' -E \
    ...
    -e "s|(href=\"widget-ds\.css)(\?v=[0-9]+)?\"|\1?v=$V\"|g" \
    "$f"
  echo "  stamped $f"
done
